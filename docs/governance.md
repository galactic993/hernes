...
## 8.5 Spec-Kit との合成（authoring は Spec-Kit、統治は hernes）

[GitHub Spec-Kit](https://github.com/github/spec-kit) は SDD のオーサリング（constitution→specify→clarify→plan→tasks→implement）に優れる。
ただし constitution は AI が「参照」する markdown、clarify/analyze は LLM レビューで、**違反で CI が落ちる決定論ゲートは持たない**（＝「書かれている」層）。
hernes governance はその上に「効いている」層を被せられる。両者は競合せず合成できる。

`make govern-speckit DIR=<Spec-Kitプロジェクト>`（[governance/src/adapters/speckit.ts](../governance/src/adapters/speckit.ts)）が
`specs/<NNN>/{spec.md,tasks.md}` と `.specify/memory/constitution.md` を読み、決定論ゲートを適用する:

| コード | 重大度 | Spec-Kit に無い「効いている」検知 |
|---|---|---|
| `SPECKIT_UNRESOLVED_CLARIFICATION` | error | `[NEEDS CLARIFICATION]` が残ったまま実装に進むのを CI で阻止（コメント/例示は除外） |
| `SPECKIT_BROKEN_TEST_PROOF` | error | 完了(`[x]`)テストタスクの参照テストファイルが実在しない（未着手 `[ ]` は対象外） |
| `SPECKIT_PLACEHOLDER_TASKS` | error | tasks.md に未置換プレースホルダ（`[name]`/`[entity]` 等）が残る |
| `SPECKIT_NO_TASKS` / `SPECKIT_NO_SPECS` | error | 要件/ストーリーがあるのにタスクが無い／検査対象ゼロ（fail-closed） |
| `SPECKIT_P1_UNTESTED` | warn | P1（最重要）ユーザーストーリーにテストタスクが無い |

→ **「SDD は Spec-Kit、統治（効いている）は hernes」が一つのリポジトリで成立する。**
